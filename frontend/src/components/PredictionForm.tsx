...
import { normalizeClinicalInput, type ClinicalHeartInput } from '../services/api';

interface Props {
  onSubmit: (data: ClinicalHeartInput) => void;
  isLoading: boolean;
}

/**
 * Only clinical model inputs. Age and sex are supplied from the user profile on the backend.
 */
const PredictionForm: React.FC<Props> = ({ onSubmit, isLoading }) => {
  ...
  return (
    <form
      onSubmit={handleSubmit((data) => onSubmit(normalizeClinicalInput(data)))}
      className="space-y-8"
    >
      <p className="text-sm text-slate-600 bg-blue-50 border border-blue-100 rounded-lg px-4 py-3">
        Age and gender are taken from your account (set at registration, editable in Settings). They
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <select
              {...register('fbs', { valueAsNumber: true })}
              className="w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none"
            >
          ...
          <div>
            ...
            <select
              {...register('restecg', { valueAsNumber: true })}
              className="w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none"
            >
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select
              {...register('exang', { valueAsNumber: true })}
              className="w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none"
            >
          ...
          <div>
            ...
            <select
              {...register('thal', { valueAsNumber: true })}
              className="w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none"
            >
